...
namespace diskann {
  ...
  MergeInsert<T,TagT>::MergeInsert(Parameters& parameters, size_t max_points, size_t dim, const std::string& mem_prefix, 
          const std::string disk_prefix_in, const std::string disk_prefix_out, Distance<T>* dist)
    ...
    _disk_index = new diskann::PQFlashIndex<T,TagT>(reader);

    std::string pq_prefix = _disk_index_prefix_in + "_pq";
    std::string disk_index_file = _disk_index_prefix_in + "_disk.index";
    int res = _disk_index->load(_num_search_threads, pq_prefix.c_str(),
                      disk_index_file.c_str(), true);
    if(res != 0) 
    {
        std::cout << "Failed to load disk index in MergeInsert constructor" << std::endl;
        exit(-1);
    }
    }
 ...
 void MergeInsert<T,TagT>::construct_index_merger()
 {
    ...
    _merger = new diskann::IndexMerger<T>(_disk_index_prefix_in.c_str(), mem_in, _disk_index_prefix_out.c_str(), _deleted_tags_file.c_str(), _dim, 
            _dist_comp, _beamwidth, range, l_index, alpha, maxc);
    std::cout << "Created index merger object" << std::endl;
 ...
 template<typename T, typename TagT>
 int MergeInsert<T, TagT>::insert(const T* point, const TagT& tag, int & percentile)
 {
     while(_check_switch_index.load())
     {
         ...
         {
             std::cout << "Active index indicated as _mem_index_0 but it cannot accept insertions" << std::endl;
             return -1;
         }
         if((_active_index == 1) && (_active_1.load() == false))
         {
             std::cout << "Active index indicated as _mem_index_1 but it cannot accept insertions" << std::endl;
             return -1;
         }
         ...
         if(_active_index == 0)
         {
             if(_mem_index_0->get_num_points() < _mem_index_0->return_max_points())
             {
                 ...
                 {
                     std::cout << "Could not insert point with tag " << tag << std::endl;
                     return -3;
                 }
                 {
         ...
         else
         {
             ...
             {
                 std::cout << "Capacity exceeded in mem_index 1" << std::endl;
             }
         }
 ...
 void MergeInsert<T,TagT>::lazy_delete(const TagT& tag)
 {
      ...
      {
	      std::cout << "Active deletion set indicated as _deletion_set_0 but it cannot accept deletions" << std::endl;
      }
      if((_active_delete_set == 1) && (_active_del_1.load() == false))
      {
	      std::cout << "Active deletion set indicated as _deletion_set_1 but it cannot accept deletions" << std::endl;
      }
...
/* template<typename T, typename TagT>
     void MergeInsert<T,TagT>::lazy_delete(tsl::robin_set<TagT> & delete_list)
     {
         std::unique_lock<std::shared_timed_mutex> lock(_delete_lock);
         _deletion_set = delete_list;
     }
*/
 template<typename T, typename TagT>
                     ...
                     TagT* tags, float * distances, QueryStats * stats)
    {
        ...
        _disk_index->cached_beam_search(query, search_L, search_L,disk_result_ids_64.data(), disk_result_dists.data(), _beamwidth, stats,nullptr, disk_result_tags.data());
...
//                    best.insert(Neighbor_Tag<TagT>(disk_result_tags[i], disk_result_dists[i]));
        ...
        //check each memory index - if non empty and not being currently cleared - search and get top K active tags 
        {
            if(_clearing_index_0.load() == false) 
            {
                ...
                _mem_index_0->search(query, search_L, search_L, best_mem_index_0);
            ...
            if(_clearing_index_1.load() == false) 
            {
                ...
                _mem_index_1->search(query, search_L, search_L, best_mem_index_1);
...
//          best_vec.erase(best_vec.begin() + K, best_vec.end());
 ...
 {
     std::cout << "Inside final_merge()." << std::endl;
     std::cout << _mem_index_0->get_num_points() << "  " << _mem_index_1->get_num_points() << std::endl;
     if(_mem_points > 0)
     {
         save_del_set();
         switch_index();
 ...
 void MergeInsert<T,TagT>::merge()
 {
    ...
    std::unique_lock<std::shared_timed_mutex> lock(_disk_lock);
            ...
            if (_switching_disk_prefixes.compare_exchange_strong(expected_value, true)) {
              std::cout << "Switching to latest merged disk index " << std::endl;
            } else {
...
//              return -1;
    ...
    if(res != 0)
    {
        std::cout << "Failed to load new disk index after merge" << std::endl;
        exit(-1);
    }
    expected_value = true;
     ...
     //and then change the value of all related flags
     {
        ...
        if(_active_index == 0)
        {
            ...
            if (_active_1.compare_exchange_strong(expected_active, true)) {
              ...
              std::cout << "Failed to initialise new _mem_index_1" << std::endl;
//              return -1;
            }
        ...
        else
        {
            ...
            if (_active_0.compare_exchange_strong(expected_active, true)) {
              ...
              std::cout << "Failed to initialise new _mem_index_0" << std::endl;
  //            return -1;
            }
        }
     ...
     std::cout << "Merge time : " << timer.elapsed()/1000 << " ms" << std::endl;
     //end timer
 ...
 int MergeInsert<T,TagT>::save()
 {
    ...
    if (_active_0.compare_exchange_strong(expected_active, false)) {
      std::cout << "Saving mem index 0 to merge it into disk index" << std::endl;
      std::string save_path = _mem_index_prefix + "_0";
      _mem_index_0->save(save_path.c_str(), true, true);
    } else {
      std::cout << "Index 0 is already inactive" << std::endl;
      return -1;
    }
    }
    else
    ...
    if (_active_1.compare_exchange_strong(expected_active, false)) {
      std::cout << "Saving mem index 1 to merge it into disk index" << std::endl;
      std::string save_path = _mem_index_prefix + "_1";
      _mem_index_1->save(save_path.c_str(), true, true);
    } else {
      std::cout << "Index 1 is already inactive" << std::endl;
      return -1;
    }
    }
    std::cout << "Saved mem index" << std::endl;
    return 0;
 }
    ...
    {
	    {
		    ...
		    if(_active_delete_set == 0)
		    {
			     ...
			     if(_active_del_1.compare_exchange_strong(expected_active, true)) {
				     std::cout << "Cleared _deletion_set_1 - ready to accept new points" << std::endl;
			     }
			     else
			     {
				     std::cout << "Failed to clear _deletion_set_1" << std::endl;
			   	}
		     }
		     else
		     {
			     ...
			     if(_active_del_0.compare_exchange_strong(expected_active, true)) {
				     std::cout << "Cleared _deletion_set_0 - ready to accept new points" << std::endl;
			     }
			     else
			     {
				     std::cout << "Failed to clear _deletion_set_0" << std::endl;
			     }
		     }
	 ...
	 if(_active_delete_set == 0)
	 {
		 TagT * tag_data = new TagT[_deletion_set_1.size()];
		 size_t i = 0;
		 for(auto iter : _deletion_set_1)
			...
			tag_data[i] = iter;
         	...
         	diskann::save_bin<TagT>(_deleted_tags_file, tag_data, _deletion_set_1.size(), (size_t)1);
	        delete[] tag_data;
	 }
	 else
	 {
		 TagT * tag_data = new TagT[_deletion_set_0.size()];
		 size_t i = 0;
		 for(auto iter : _deletion_set_0)
			...
			tag_data[i] = iter;
         	...
         	diskann::save_bin<TagT>(_deleted_tags_file, tag_data, _deletion_set_0.size(), (size_t)1);
	        delete[] tag_data;
	 }
