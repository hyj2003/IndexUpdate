...
//    std::shared_ptr<AlignedFileReader> reader = std::make_shared<MemAlignedFileReader>();
  ...
  //    Index<T, TagT>* mem_index = new Index<T, TagT>(Metric::L2, bin_ndims, bin_npts + 100, 1, true);
      ...
      mem_index->load(ind_path.c_str(), data_path.c_str(), true, nullptr, true);
      _u64 n1, n2, n3;
      ...
      std::cout << "Reading memory index ..." << std::endl;
      mem_index->load_tag_bin(mem_index_path + ".tags");
      std::cout << " Reading memory index done." << std::endl;
...
//        std::vector<uint8_t> j_pq_coords(this->pq_nchunks,0);
  ...
  void IndexMerger<T, TagT>::process_inserts() {
    ...
    std::cout << "Medoid num: " << this->disk_index->GetNumMedoids() << std::endl;
    // iterate through each vector in each mem index
    for(uint32_t i=0;i<this->mem_data.size(); i++) {
    ...
    std::cout << "Cache hit = " << this->disk_index->cache_hit << " / " << this->disk_index->total_travel << std::endl;
    // for (int i = 0; i < 40; i++) {
    //   std::cout << "[";
    //   for (auto j : this->disk_index->traveled[i]) {
    //     std::cout << j << ", ";
    //   }
    //   std::cout << "]\n";
    // }
  ...
  void IndexMerger<T, TagT>::optimized_process_inserts() {
    ...
    //  const T* coords = this->mem_data[i];
      ...
      const uint32_t offset = this->offset_ids[i];
      const uint32_t count = this->mem_npts[i];
      const std::vector<std::vector<uint32_t>> &g = *this->mem_graphs[i];
      auto Bfs = [&g, count]() -> std::vector<_u32> {
        const size_t deg = 64;
        Timer timer;
        std::queue<_u32> q;
        std::vector<bool> vis(count, false);
        vis.reserve(count);
        std::vector<_u32> seq;
        seq.reserve(count);
        _u32 start_point = 0, max_degree = g[0].size();
        ...
        std::cout << "Bfs done in " << (float)timer.elapsed() / 1000 / 1000 
                  << "s, travelled " << std::accumulate(vis.begin(), vis.end(), int(0)) << " in total.\n";
        for (size_t i = 0; i < count; i++) {
          if (!vis[i]) seq.emplace_back(i);
        }
        return std::move(seq);
      };
      std::vector<_u32> cnt(count, 0);
      for (size_t i = 0; i < count; i++) {
        for (auto j : g[i]) {
          cnt[j]++;
        }
      ...
      std::cout << "max in-degree: " << *std::max_element(cnt.begin(), cnt.end()) 
                ...
                << std::endl;
      std::vector<_u32> seq = Bfs();
      size_t cur_cache_size = 0;
      MallocExtension::instance()->GetNumericProperty("tcmalloc.max_total_thread_cache_bytes", &cur_cache_size);
      std::cout << "Current cache size : " << (cur_cache_size >> 10) << " KiB\n" << std::endl;
      MallocExtension::instance()->SetNumericProperty("tcmalloc.max_total_thread_cache_bytes", 128 * 1024 * 1024);

    // auto insert_cache = this->disk_index->CreateInsertCache(200000, MAX_INSERT_THREADS);
    // #pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_INSERT_THREADS)
    //   // iteratively insert each point into full index
    //   for(uint32_t j=0;j < count; j++) {
    //     // filter out -- `j` is deleted
    ...
    //     const uint32_t j_id = offset + j;
    //     // insert into index
    //     this->optimized_insert_mem_vec(j_coords, j_id);
    //   }
      this->disk_index->SetEntersNum(0);
      this->disk_index->CreateThreadPoints(MAX_INSERT_THREADS);
      size_t pos = 0;
      std::vector<_u32> batch; 
      batch.reserve(MAX_INSERT_THREADS);
      while (pos < seq.size()) {
        batch.clear();
        while (pos < seq.size() && batch.size() < MAX_INSERT_THREADS) {
          if (deleted_set.find(seq[pos]) == deleted_set.end()) {
            batch.emplace_back(seq[pos]);
          }
          pos++;
        }
      #pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_INSERT_THREADS)
        for (size_t j = 0; j < batch.size(); j++) {
          const T* j_coords = coords + ((uint64_t)(this->aligned_ndims) * (uint64_t)batch[j]);
          const uint32_t j_id = offset + batch[j];
          uint32_t x_id, y_id;
          // insert into index
          // std::cout << j << std::endl;
          _u32 t_id = omp_get_thread_num();
          std::vector<_u32> enters;
          for (auto x : g[batch[j]]) {
            x_id = offset + x;
            auto iter = this->disk_index->nearest_pt_.find(x_id);
            if (iter != this->disk_index->nearest_pt_.end()) {
              enters.emplace_back(iter->second);
            }
            for (size_t y = 0; y < std::min(g[x].size(), size_t(10)); y++) {
              y_id = offset + g[x][y];
              iter = this->disk_index->nearest_pt_.find(y_id);
              if (iter != this->disk_index->nearest_pt_.end()) {
                enters.emplace_back(iter->second);
              }
            }
            if (enters.size() >= size_t(256)) {
              break;
            }
          }
          // std::cout << j << " " << enters.size() << std::endl;
          this->optimized_insert_mem_vec(j_coords, j_id, t_id, enters);
          if(((pos + j) % 100000 == 0) && (pos + j > 0)) {
		        std::cout << "Finished inserting " << pos + j << " points" << std::endl;
            // std::cout << insert_cache->q_.size() << " " << insert_cache->cur_ << " " << insert_cache->cap_ << std::endl;
          }
        }
        this->disk_index->SetEntersNum(batch.size());
        this->disk_index->UpdateThreadPoints(batch.size());
        for (size_t j = 0; j < batch.size(); j++) {
          this->disk_index->nearest_pt_[offset + batch[j]] = this->disk_index->insert_enters_[j];
        }
        // insert_cache->MergeSearchInfo();
      }
      // this->disk_index->DeleteInsertCache();
    }
    this->disk_index->nearest_pt_.clear();
    std::cout << "Finished inserting all points\n";
    double e2e_time = ((double)total_insert_timer.elapsed())/(1000000.0);
    double insert_time = std::accumulate(this->insert_times.begin(), this->insert_times.end(), 0.0);
    double delta_time = std::accumulate(this->delta_times.begin(), this->delta_times.end(), 0.0);
    ...
    std::cout <<" E2E insert time: "<< e2e_time << " sec" << std::endl;
    std::cout << "Cache hit = " << this->disk_index->cache_hit << " / " << this->disk_index->total_travel << std::endl;
    std::cout << "Distance = " << this->disk_index->dist_1 << " / " << this->disk_index->dist_2 << std::endl;
    // for (int i = 0; i < 40; i++) {
    //   std::cout << "[";
    //   for (auto j : this->disk_index->traveled[i]) {
    //     std::cout << j << ", ";
  ...
  void IndexMerger<T, TagT>::optimized_insert_mem_vec(const T* mem_vec, 
    ...
    // std::cout << "p1" << std::endl;
    // search on combined graph
    this->cached_offset_iterate_to_fixed_point(mem_vec, offset_id, this->l_index, pool, coord_map, t_id, enters);
    // std::cout << "p2" << std::endl;
    insert_time = timer.elapsed();
...
//    ThreadData<T> * thread_data = nullptr;
    ...
    this->disk_index->cached_disk_iterate_to_fixed_point(vec, Lsize, this->beam_width, 
                                                         exp_node_info, &cmap, nullptr, 
                                                         nullptr, &thread_data, 
                                                         &this->disk_deleted_ids, t_id, enters);
...
//    expanded_nodes_info.clear();
  ...
  void IndexMerger<T, TagT>::occlude_list_pq_simd(std::vector<Neighbor> &pool, 
    ...
    const _u64 nchunks = this->pq_nchunks;
      ...
      for (size_t i = 0; i < pool.size(); i += kSize) {
        ...
        for (size_t j = 0; j < len; j++) {
          for (size_t k = 0; k < nchunks; k++) {
            pq[k * kSize + j] = pq_data[nchunks * pool[i + j].id + k];
          }
        }
      }
    }
    PQComputer<T> *computer = 
      new PQComputer(this->pq_data,
                     scratch_T,
                     std::min(pool.size(), size_t(maxc)),
                     pq_nchunks,
                     scratch,
                     disk_index->GetPQTable());
    ...
    while (cur_alpha <= alpha && result.size() < rerank_size) {
      ...
      while (result.size() < rerank_size && (start) < pool.size() && start < maxc) {
        ...
        occlude_factor[start] = std::numeric_limits<float>::max();
        result.push_back(p);
        computer->compute_batch_dists(this->pq_data + pool[start].id * nchunks, 
                                      start / 64 * 64, 
    ...
    for (auto &iter : result) {
      this->disk_index->compute_pq_dists(id, &(iter.id), &iter.distance, 1, scratch);
    }
 ...
 #pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
    ...
    *(uint64_t* )(buf + sizeof(uint64_t)) = (uint64_t) this->disk_npts;
    std::cout << "Processed Size: " << file_size << " " << (uint64_t) this->disk_npts << std::endl;
    // determine medoid
 ...
 #pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
      for(uint32_t i=0; i < disk_nodes.size(); i++) {
        ...
        int omp_thread_no = omp_get_thread_num();
        uint8_t* pq_coord_scratch = this->thread_bufs[omp_thread_no];
        uint16_t* u16_scratch = this->thread_bufs_u16[omp_thread_no];
        assert(pq_coord_scratch != nullptr);
        DiskNode<T> &disk_node = disk_nodes[i];
  ...
  void IndexMerger<T, TagT>::optimized_consolidate_deletes(DiskNode<T> &disk_node, 
    ...
    std::vector<float> id_nhood_dists(id_nhood.size(), 0.0f);
    assert(scratch != nullptr);
    // std::cout << disk_node.id << std::endl;
    {
      auto computer = new PQComputer(this->pq_data, 
                                    nullptr, 
                                    id_nhood.size(), 
                                    this->pq_nchunks, 
                                    nullptr, 
                                    this->disk_index->GetPQTable());
      computer->compute_pq_dists(id, id_nhood.data(), id_nhood_dists.data(), id_nhood.size());
    }
    // std::cout << "compute pq done." << std::endl;
    // prune neighbor list using PQ distances
    std::vector<Neighbor> cand_nbrs(id_nhood.size());
    for(uint32_t i=0;i<id_nhood.size(); i++) {
      cand_nbrs[i].id = id_nhood[i];
    ...
    std::vector<float> occlude_factor(cand_nbrs.size(), 0.0f);
    pruned_nbrs.reserve(this->range);
    size_t current_ef = std::min(size_t(this->maxc), cand_nbrs.size());
    avg_ef += current_ef;
...
//    tag_writer.close();
  ...
  void IndexMerger<T, TagT>::process_merges() {
    ...
    *(uint64_t* )(buf + 4 * sizeof(uint64_t)) = nnodes_per_sector;
    ...
    // std::cout << "io time: " << io_total << std::endl;
  }
  ...
  void IndexMerger<T, TagT>::optimized_merge() {
    ...
    this->optimized_process_deletes();
    std::cout << "Computing rename-map.\n";
    // compute rename map
    this->compute_rename_map();
    ...
    // call inserts
    this->optimized_process_inserts();
    this->process_inserts_pq();
